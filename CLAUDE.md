...
- Перед любой правкой ширины/выравнивания блока (`width`, `max-width`,
  ...
  `docs/ARCHITECTURE.md` - "Грабли верстки". Там уже пойманные ошибки и
  чек-лист проверки; это дешевле, чем сломать живой сайт и чинить по
  скриншоту от пользователя. Главное оттуда: секции главной - flex-элементы
  `.home-page__main`, и `max-width` + `margin-inline: auto` на таком блоке
  схлопывает его в shrink-to-fit; центрируется явной шириной
  `width: min(100%, N px)`.
- Сломал что-то на живом сайте - после фикса допиши случай в раздел 6.1,
  одной строкой: что сделал, что сломалось, как правильно.

## Git

Дефолтная и продакшн-ветка репозитория - `claude/design-system-prototype-gmmdk3`
(ветки `main` в этом репозитории нет и создавать ее не нужно). Коммить и
пушь изменения сразу в `claude/design-system-prototype-gmmdk3`. Не
создавай новые отдельные ветки и не открывай Pull Request - ревью кода
никто не делает, проверка идет вручную на живом сайте после деплоя.
Vercel деплоит именно эту ветку, поэтому любая другая ветка до
продакшна не доходит и правки останутся невидны при ручной проверке.
...
- Не запускай `npm run build` или `npm run dev` самостоятельно, если явно
  не попросили проверить сборку. Исключение - layout-правка: ее по
  чек-листу из 6.1 надо проверить скриншотом до пуша, потому что пуш сразу
  уходит в прод.
