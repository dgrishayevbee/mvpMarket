...
Коммить и пушь изменения сразу в `main`. Не создавай отдельные ветки
(`claude/...` и любые другие) и не открывай Pull Request - ревью кода
никто не делает, проверка идет вручную на живом сайте после деплоя.
Vercel деплоит именно `main`, поэтому любая ветка кроме `main` до
